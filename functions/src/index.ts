...
import { onDocumentWritten as onOpsWritten } from "firebase-functions/v2/firestore";
 ...
 * Fires when the dashboard writes to _ops/recalc.
 * Forces a full ALL-VISIT queue recalculation for today
 * (morning + evening sessions).
 */
export const onManualRecalc = onOpsWritten("_ops/recalc", async (event) => {
  try {
    logger.info("🔄 Manual recalculation triggered from dashboard.");
    ...
    // Recalculate both sessions
    await Promise.all([
      recalcQueueForDateSession(dateStr, "morning"),
      recalcQueueForDateSession(dateStr, "evening")
    ]);

    logger.info(`Manual recalculation finished for ${dateStr}`);

  } catch (err) {
    logger.error("❌ Error running manual recalculation:", err);
  }
});
