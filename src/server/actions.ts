...
import { calculateETCsForQueue } from '@/lib/queue-utils';
...
// Approximate clinic coordinates for 100m radius validation
...
/** 
 * ==============================================================
 * SMART QUEUE RECALCULATION + CONTROLLED ADVANCEMENT
 * Updated: 2025-10-20 by GPT-5 for ShantiQ Queue Logic
 * ==============================================================
 */
/**
 * Utility: ensure a valid numeric latePenalty value (default 3 if not set)
 */
function getLatePenalty(p: Patient): number {
  if (typeof p.latePenalty === 'number' && p.latePenalty >= 1) {
    return p.latePenalty;
  }
  // Default mild penalty if missing
  return 3;
}
...
export async function recalculateQueueWithETC(): Promise<ActionResult> {
  ...
  // Helper to compute session membership for a UTC time
  const sessionOf = (utcDate: Date): 'morning'|'evening'|null => {
    return getSessionForTime(
      { ...schedule, days: { ...schedule.days, [dayOfWeek]: daySchedule } } as DoctorSchedule,
      utcDate
    );
  };
  ...
  for (const session of sessions) {
    ...
    const sessionStartUtc = sessionLocalToUtc(todayStr, sessionTimes.start);
    const sessionEndUtc   = sessionLocalToUtc(todayStr, sessionTimes.end);

    // Collect TODAY’s patients in this session (ignore cancelled)
    const sessionPatients = allPatients.filter(p => {
      if (!p.appointmentTime || p.status === 'Cancelled') return false;
      const appt = parseISO(p.appointmentTime);
      const sameDay = format(toZonedTime(appt, timeZone), 'yyyy-MM-dd') === todayStr;
      return sameDay && sessionOf(appt) === session;
    });
    ...
    if (doctorStatus.isOnline && doctorStatus.onlineTime) {
      ...
      if (sessionOf(onlineTimeUtc) === session) {
        sessionDelay = Math.max(0, differenceInMinutes(onlineTimeUtc, sessionStartUtc));
      }
    } else if (doctorStatus.startDelay > 0) {
      // Apply configured start delay if session not started yet
      sessionDelay = doctorStatus.startDelay;
    }
    const delayedSessionStartUtc = addMinutes(sessionStartUtc, sessionDelay);

    // ----- Calculate each patient’s original slot time from token (stable baseline) -----
    sessionPatients.forEach(p => {
      if (!p.tokenNo) return;
      const slotUtc = addMinutes(delayedSessionStartUtc, (p.tokenNo - 1) * schedule.slotDuration);
    ...
    // ----- Auto mark LATE (strict rules you asked) -----
    // Applies only to portal appointments or booked walk-ins,
    // only if they CHECKED IN after missing their slot.
    sessionPatients.forEach(p => {
      const isBookedType = (p.type === 'Appointment') || (p.subType === 'Booked Walk-in');
      if (!isBookedType || !p.checkInTime || !p.tokenNo) return;

      const slot = patientUpdates.get(p.id)?.slotTime
        ? parseISO(patientUpdates.get(p.id)!.slotTime as string)
        : addMinutes(delayedSessionStartUtc, (p.tokenNo - 1) * schedule.slotDuration);

      const checkedIn = parseISO(p.checkInTime);
      const missedSlot = checkedIn.getTime() > slot.getTime();

      if (missedSlot && p.status === 'Waiting' && !p.lateLocked) {
        const lateBy = Math.max(0, differenceInMinutes(checkedIn, slot));
        patientUpdates.set(p.id, { ...patientUpdates.get(p.id), status: 'Late', lateBy });
      }
    });

   // ----- Build queue with stable order and late-penalty retention -----
...
// Separate normal waiting and penalized
const normalWaiting = sessionPatients
...
// Handle penalized (late) patients → respect anchors dynamically
const penalized = sessionPatients.filter(p => p.lateLocked);

const baseQueue: Patient[] = [...normalWaiting];
for (const late of penalized) {
  const anchorIds = (late.lateAnchors || []).filter(id => baseQueue.find(p => p.id === id));
  const lastAnchorIdx = Math.max(-1, ...anchorIds.map(id => baseQueue.findIndex(p => p.id === id)));
  const penalty = getLatePenalty(late);
  const insertAt = lastAnchorIdx >= 0 ? lastAnchorIdx + 1 : Math.min(penalty, baseQueue.length);
  baseQueue.splice(insertAt, 0, late);
}

// Promote Up-Next if missing
if (!currentUpNext && baseQueue.length > 0) {
  currentUpNext = baseQueue[0];
  patientUpdates.set(currentUpNext.id, { ...(patientUpdates.get(currentUpNext.id) || {}), status: 'Up-Next' });
}

// Combine into liveQueue for ETC computation
const liveQueue: Patient[] = [];
if (inConsultation) liveQueue.push(inConsultation);
if (currentUpNext) liveQueue.push(currentUpNext);
for (const p of baseQueue) if (p.id !== currentUpNext?.id) liveQueue.push(p);

// Compute dynamic ETCs using central utility
const delayedClinicStartTime = delayedSessionStartUtc;
const etcMap = calculateETCsForQueue(
  liveQueue,
  schedule,
  doctorStatus,
  delayedSessionStartUtc,
  inConsultation
);

etcMap.forEach((etc, id) => {
  patientUpdates.set(id, {
    ...(patientUpdates.get(id) || {}),
    bestCaseETC: etc.bestCaseETC,
    worstCaseETC: etc.worstCaseETC,
  });
});

    // If there is no Up-Next and the queue has someone waiting, promote the first in queue
    if (!currentUpNext && baseQueue.length > 0) {
      currentUpNext = baseQueue[0];
      patientUpdates.set(currentUpNext.id, { ...patientUpdates.get(currentUpNext.id), status: 'Up-Next' });
    }

    // Build the live order used for ETC calculation
    const liveOrder: Patient[] = [];
    if (inConsultation) liveOrder.push(inConsultation);
    if (currentUpNext)  liveOrder.push(currentUpNext);
    baseQueue.forEach(p => { if (p.id !== currentUpNext?.id) liveOrder.push(p); });

   // ✅ Use central queue-utils for consistent ETC computation
...
etcMap.forEach((etc, id) => {
  ...
  if (patientUpdates.size > 0) {
    ...
    patientUpdates.forEach((updates, id) => batch.update(doc(db, 'patients', id), updates));
    await batch.commit();
  }
...
/** =============================================================
 * Move current queue forward manually (used by “Consult Next”)
 * ============================================================= */
export async function advanceQueueAction(upNextPatientId: string): Promise<ActionResult> {
  // 1) Complete current In-Consultation (if any)
  let all = await getPatientsData();
  const nowServing = all.find(p => p.status === 'In-Consultation');

  if (nowServing?.consultationStartTime) {
    const start = parseISO(nowServing.consultationStartTime);
    const end = new Date();
    await updatePatient(nowServing.id, {
      status: 'Completed',
      consultationEndTime: end.toISOString(),
      consultationTime: Math.max(0, differenceInMinutes(end, start)),
      subStatus: undefined,
      lateLocked: false,
      lateAnchors: undefined,
      lateLockedAt: undefined,
  ...
  // 2) Move the designated Up-Next into In-Consultation
  await updatePatient(upNextPatientId, {
  ...
  // 3) Recalculate to promote the next Up-Next & refresh ETC
  await recalculateQueueWithETC();
  revalidatePath('/', 'layout');
...
/** =============================================================
 * Handle the final patient (when queue nearly empty)
 * ============================================================= */
export async function startLastConsultationAction(patientId: string): Promise<ActionResult> {
  ...
  if (nowServing && nowServing.consultationStartTime) {
    ...
    await updatePatient(nowServing.id, {
      status: "Completed",
      consultationEndTime: end.toISOString(),
      consultationTime: Math.round((end.getTime() - start.getTime()) / 60000),
    });
  }

  await updatePatient(patientId, {
    status: "In-Consultation",
    consultationStartTime: now.toISOString(),
    consultNext: false,
  });

  await recalculateQueueWithETC();
  revalidatePath("/", "layout");
  return { success: "Final consultation started." };
}
...
export async function patientImportAction(familyData: FormData, childData: FormData): Promise<ActionResult> {
    ...
    const parseCsv = async (file: File): Promise<any[]> => {
        ...
        return data.filter(row => Object.values(row).some(val => val)); // Filter out empty rows
    };

    try {
        const familyFile = familyData.get('file') as File;
        const childFile = childData.get('file') as File;

        if (!familyFile || !childFile) {
            return { error: "Both family and child CSV files are required." };
        }

        const familyJson = await parseCsv(familyFile);
        const childJson = await parseCsv(childFile);

        const familyResult = await batchImportFamilyMembers(familyJson);
        const childResult = await batchImportFamilyMembers(childJson);

        return { success: `Imported ${familyResult.successCount} families and ${childResult.successCount} children. Skipped ${familyResult.skippedCount + childResult.skippedCount} duplicates.` };
